...
**Origin & статус.** Проект начат как дипломная работа под дедлайн. Сейчас идёт доводка под более высокие стандарты: единый контракт API, миграции БД, безопасность (bcrypt/JWT/.env), базовые тесты и CI. В БД встречаются как легаси, так и bcrypt-хэши — см. «Безопасность» и «Миграции».

### Демо / Скриншоты
- **Демо:** coming soon
- **Скриншоты:** `./docs/screenshots/` *(WIP)*

### Highlights (что сделано руками)
- **Auth & roles:** JWT-авторизация + разграничение доступа (parent/child/admin)
- **Client–server:** React (CRA) ↔ Express REST API ↔ MySQL
...
## Безопасность
- **Пароли:** перейти полностью на bcrypt (12 rounds). Тактика: «ленивая миграция» — при первом логине пользователя с легаси хешированием перехэшировать bcrypt и сохранить; запретить новые легаси варианты.
...
## Текущие нюансы
- Смешанные схемы хеширования (легаси и bcrypt) → миграция в процессе
